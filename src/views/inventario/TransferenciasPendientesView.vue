  ...
  <div class="flex flex-col gap-6">
    ...
    <template v-if="!selectedRecibo">
      ...
      <section class="rounded-[14px] border border-amber-200 bg-amber-50 px-6 py-4">
        <h2 class="text-sm font-semibold text-amber-900">Bandeja de transferencias de inventario</h2>
        <p class="mt-1 text-xs text-amber-700">
          Recibos de inventario pendientes de aprobación. Haz clic en un registro para revisar el comprobante.
        </p>
      </section>
      ...
      <div v-else-if="!pendientes.length" class="rounded-[14px] border border-black/10 bg-white py-16 text-center">
        <p class="text-sm text-slate-500">No hay transferencias de inventario pendientes de aprobación.</p>
      </div>

      <!-- Tabla -->
      <template v-else>
        <p class="text-xs text-slate-500">
          {{ pagination.total }} recibo{{ pagination.total !== 1 ? 's' : '' }} pendiente{{ pagination.total !== 1 ? 's' : '' }}.
        </p>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              ...
              <tbody class="divide-y divide-black/5">
                ...
                >
                  <td class="px-4 py-3 font-medium text-slate-900">{{ recibo.estudiante?.name ?? '—' }}</td>
                  ...
                  <td class="px-4 py-3 text-right font-mono font-semibold text-slate-900">$ {{ formatMoney(recibo.valor_total) }}</td>
                  <td class="px-4 py-3 text-right">
                    <span class="whitespace-nowrap text-xs font-medium text-amber-600">Ver detalle →</span>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <template v-if="selectedMp?.comprobante_url">
            ...
            <template v-else>
              <div class="flex flex-col items-center justify-center gap-4 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 py-14">
                ...
                <a :href="resolveStorageUrl(selectedMp.comprobante_url)" target="_blank" rel="noopener" class="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                  Abrir documento
                </a>
              </div>
            </template>
        ...
        <div class="flex flex-col gap-4">
          ...
          <div class="rounded-[14px] border border-black/10 bg-white p-6">
            ...
            <dl class="space-y-3">
              ...
              <div class="flex items-baseline justify-between pt-1">
                ...
                <dd class="font-mono text-2xl font-bold text-[#213360]">$ {{ formatMoney(selectedRecibo.valor_total) }}</dd>
              </div>
            </dl>
          </div>
          ...
          <div class="rounded-[14px] border border-black/10 bg-white p-6">
            ...
            <p class="mb-4 text-xs text-slate-500">
              Al aprobar se asigna el número de recibo de inventario. Al rechazar, el cajero podrá corregir y reenviar.
            </p>
    ...
    <ModalBase
      ...
      title="Aprobar transferencia de inventario"
      description="Se asignará el número de recibo de inventario y el recibo quedará cerrado."
    >
      <div class="space-y-3 pb-2">
        ...
        <p class="text-xs text-slate-500">Esta acción solo afecta la numeración de inventario; no modifica la cartera académica.</p>
      </div>
      <template #footer>
        ...
        <button type="button" :disabled="aprobandoId !== null" class="flex items-center gap-2 rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-green-700 disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-green-500" @click="doAprobar">
          <svg class="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
    ...
    >
      <div class="space-y-4 pb-2">
        <p class="text-sm text-slate-700">
          ¿Confirmas rechazar el recibo de
          <strong>{{ reciboTarget?.estudiante?.name }}</strong>
          por <strong>$ {{ formatMoney(reciboTarget?.valor_total) }}</strong>?
        </p>
        <div>
          <label for="motivo-rechazo-inv" class="mb-1 block text-sm font-medium text-slate-700">
            Motivo del rechazo <span class="text-red-500">*</span>
          </label>
          <textarea
            id="motivo-rechazo-inv"
        ...
        <div v-if="modalError" class="rounded-lg bg-red-50 p-3 text-sm text-red-700">{{ modalError }}</div>
      </div>
      <template #footer>
        <button type="button" class="rounded-lg px-4 py-2 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500" @click="showRechazarModal = false">Cancelar</button>
        <button type="button" :disabled="rechazando || !motivoRechazo.trim()" class="rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700 disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-red-500" @click="confirmRechazar">
...
async function loadPendientes(page = 1) {
  ...
  error.value   = ''
  try {
    // Filtramos por origen=0 (INVENTARIOS) para separar de los recibos académicos
    const params = { page, per_page: 15, origen: 0 }
...
async function doAprobar() {
  ...
  try {
    const res = await invVentaService.aprobarTransferencia(recibo.id)
...
async function confirmRechazar() {
  ...
  modalError.value = ''
  try {
    await invVentaService.rechazarTransferencia(reciboTarget.value.id, motivoRechazo.value.trim())
